...
$filename = "contact_" . date('Ymd-His') . ".xls";
...
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'Corresponding Carpark Site')
	        ->setCellValue('B1', 'Email Address')
            ->setCellValue('C1', 'Contact Person')
            ->setCellValue('D1', 'Contact Subject')
...
if(is_array($displayData["List"]) && count($displayData["List"]) > 0)
{
    ...
    foreach($displayData["List"] as $id => $value)
    {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A'.$index, $value["correspondingcarparksite"])
	        ->setCellValue('B'.$index, $value["emailaddress"])
            ->setCellValue('C'.$index, $value["contactperson"])
            ->setCellValue('D'.$index, $value["contactsubject"])
            ->setCellValue('E'.$index, $value["contactcontent"])
    ...
    $objPHPExcel->getActiveSheet()->getStyle('A1:F'.(count($displayData["List"]) + 1))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
    $objPHPExcel->getActiveSheet()->getStyle('A1:F'.(count($displayData["List"]) + 1))->getAlignment()->setWrapText(true); 
    ...
    foreach($displayData["List"] as $id => $value)
    {
         ...
         $objPHPExcel->getActiveSheet()
             ->setCellValueExplicit('C'.($xo+1), $value["contactperson"], PHPExcel_Cell_DataType::TYPE_STRING);
        // $objPHPExcel->getActiveSheet()
